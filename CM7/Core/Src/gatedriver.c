...
//                                            |                    |                |
#define SAMPLE_TO_AMPS \
	((3.3f / ADC_MAX_VALUE) * (1.0f / 1.53f) * (1.0f / 0.33f))
		  ...
		  ADC_HandleTypeDef *phase_adc, foc_ctrl_t *controller)
{
	...
	assert(HAL_TIM_PWM_ConfigChannel(gatedriver->tim, &channel_config,
...
void gatedrv_write_pwm(gatedriver_t *drv, float duty_cycles[GATEDRV_NUM_PHASES])
{
	...
	drv->tim->Instance->CCR1 =
		(uint32_t)(duty_cycles[0] * PWM_PERIOD_CYCLES) / 2;
	drv->tim->Instance->CCR2 =
		(uint32_t)(duty_cycles[1] * PWM_PERIOD_CYCLES) / 2;
	drv->tim->Instance->CCR3 =
		(uint32_t)(duty_cycles[2] * PWM_PERIOD_CYCLES) / 2;
}
...
void gatedrv_get_phase_currents(gatedriver_t *drv, float phase_currents[3])
{
	...
	phase_currents[0] =
		(float)(drv->phase_adc->Instance->JDR1 & ADC_MAX_VALUE);
	phase_currents[1] =
		(float)(drv->phase_adc->Instance->JDR2 & ADC_MAX_VALUE);
	phase_currents[2] =
		(float)(drv->phase_adc->Instance->JDR3 & ADC_MAX_VALUE);
	...
	phase_currents[0] =
		SAMPLE_TO_AMPS * (phase_currents[0] - drv->channel_offsets[0]);
	phase_currents[1] =
		SAMPLE_TO_AMPS * (phase_currents[1] - drv->channel_offsets[1]);
	phase_currents[2] =
		SAMPLE_TO_AMPS * (phase_currents[2] - drv->channel_offsets[2]);
}
...
void vPhaseActor(void *pv_params)
{
	...
	for (;;) {
		status =
			osMessageQueueGet(gatedriver->controller->command_queue,
					  &duty_cycles, NULL, osWaitForever);
		if (status == osOK) {
