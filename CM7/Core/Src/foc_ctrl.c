...
{
	controller->data_queue =
		osMessageQueueNew(INBOUND_QUEUE_SIZE, sizeof(foc_data_t), NULL);
	controller->command_queue = osMessageQueueNew(
		OUTBOUND_QUEUE_SIZE, sizeof(pwm_signal_t[3]), NULL);

	/* Initialize Clarke Transform */
	CLARKE_setNumSensors(&controller->clarke_transform,
			     NUM_CURRENT_SENSORS);
	CLARKE_setScaleFactors(&controller->clarke_transform, CLARKE_ALPHA,
			       CLARKE_BETA);
...
osStatus_t foc_retrieve_cmd(foc_ctrl_t *controller, pwm_signal_t duty_cycles[3])
{
	...
	return osMessageQueueGet(controller->command_queue, duty_cycles, NULL,
				 osWaitForever);
}
...
static void open_loop_ctrl(foc_ctrl_t *controller)
{
	...
	uint32_t dt = (uint32_t)((uint32_t)current_time_us -
				 (uint32_t)controller->last_run_us);
	if ((int32_t)dt < 0)
		dt = 180; /* Account for wrap around of timer */

	//printf("Time us: %ld\r\n", dt);

	/* Ramp amplitude */
	controller->open_loop_amplitude +=
		((OPEN_LOOP_MAX_AMPLITUDE) / OPEN_LOOP_RAMP_TIME) *
		(dt / 1000000.0);
	...
	duty_cycles[0] = controller->open_loop_amplitude *
			 (sinf(controller->open_loop_ramp_position) + 1) / 2.0 *
			 10.0;
	duty_cycles[1] =
		controller->open_loop_amplitude *
		(sinf(controller->open_loop_ramp_position - 2.0 * M_PI / 3.0) +
		 1.0) /
		2 * 10.0;
	duty_cycles[2] =
		controller->open_loop_amplitude *
	...
	controller->open_loop_ramp_position +=
		controller->open_loop_ramp_velocity * (dt / 1000000.0f);
...
static void closed_loop_ctrl(foc_ctrl_t *controller, foc_data_t *msg)
{
	...
	case FOCDATA_PHASE_CURRENT:
		...
		CLARKE_run(&controller->clarke_transform,
			   msg->payload.phase_currents, alpha_beta);
		...
		/*
			...
			*/
		PID_run_parallel(&controller->q_pid, 0, id_iq[D], 0,
				 &id_iq_pid[D]);
		PID_run_parallel(&controller->d_pid, controller->ref_current,
				 id_iq[Q], 0, &id_iq_pid[Q]);
	...
	case FOCDATA_ROTOR_POSITION:
		PARK_setup(&controller->park_transform,
			   msg->payload.rotor_position);
		IPARK_setup(&controller->ipark_transform,
			    msg->payload.rotor_position);
...
void vFOCctrl(void *pv_params)
{
	...
	for (;;) {
		...
		status = osMessageQueueGet(controller->data_queue, &msg, NULL,
					   osWaitForever);
		if (status == osOK) {
			switch (get_state()) {
			case START:
