...
static void closed_loop_ctrl(foc_ctrl_t *controller, float phase_currents[3], float duty_cycles[3])
{
	...
	// printf("alpha_beta: [%6.3f,%6.3f] ", alpha_beta[0], alpha_beta[1]);
	PARK_run(&controller->park_transform, alpha_beta, id_iq);
	// printf("id_iq: [%6.3f,%6.3f] ", id_iq[0], id_iq[1]);

	/*
	* Here, adjust I_q based on the desired current, reference for I_d is always 0
	* I_q is actually being varied
	*/
	PID_run_parallel(&controller->d_pid, 0, id_iq[D], 0, &id_iq_pid[D]);
	PID_run_parallel(&controller->q_pid, controller->ref_current, id_iq[Q], 0, &id_iq_pid[Q]);
	// printf("id_iq_pid: [%6.3f,%6.3f] ", id_iq_pid[0], id_iq_pid[1]);

	IPARK_run(&controller->ipark_transform, id_iq_pid, alpha_beta);
	// printf("alpha_beta_cmd: [%6.3f,%6.3f] ", alpha_beta[0], alpha_beta[1]);
	SVGEN_run(&controller->svm, alpha_beta, v_abc_pu);
	// printf("vabc: [%6.3f,%6.3f,%6.3f] ", v_abc_pu[0], v_abc_pu[1], v_abc_pu[2]);
